...
2. 在該活動頁面當中的地圖 view，選「GPX」下載圖示。
   ...
   ![Strava 活動頁的地圖右上角，紅框處就是「GPX」下載按鈕](docs/images/30_strava_gpx_button.png)
...
- **GPS 異常**：明顯不合理的單秒位移會被排除在速度計算之外（定位凍住幾秒後一次跳過去、或單點噪訊），避免一個壞點變成 100 km/h 的「最高速」；跳點特別大的地方會標成灰色的 GPS 飄移路段。這些位移仍然算進總距離（Strava 也是這樣算）。
